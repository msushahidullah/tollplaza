...
@section('title',__('staticwords.YourWishlist').' ('.$wishcount.') | Emart')
@section("body")
...
{{-- <div class="filter-buttons">
  <div class="list-view-button btn btn-primary"><i class="fa fa-bars" aria-hidden="true"></i> List view</div>
  <div class="grid-view-button btn btn-primary"><i class="fa fa-th-large" aria-hidden="true"></i> Grid view</div>
</div> --}}
    ...
    <div class="my-wishlist-page">
      <div class="row">
        <div class="col-md-12 my-wishlist">
          ...
          <hr >
          @if(count($data) > 0)
          <ol class="list1 list-view-filter">
          @foreach($data as $p)
            @if($p->variant && $p->variant->products->status == 1)
              <div id="orivar{{ $p->variant->id }}" class="row grid-list align-items-center">
              ...
              <hr class="orivar{{ $p->variant->id }}">
            @endif
            @if(isset($p->simple_product) && $p->simple_product->status == '1')
            <div class="row align-items-center" id="orivar{{ $p->simple_product->id }}">
              <div class="col-md-2 col-4">
              ...
              <div class="col-md-1 col-3">
                <!-- <a class="add_in_wish_simple" data-proid="{{ $p->simple_product->id }}" data-status="{{ inwishlist($p->simple_product->id) }}" data-toggle="tooltip" data-placement="right"
                ...
                @php
                $ifinwishlist = App\Wishlist::where('user_id',Auth::user()->id)->where('simple_pro_id',$p->simple_product->id)->first();
                @endphp
                @if(!empty($ifinwishlist))
                <a mainid="{{ $p->simple_product->id }}" data-remove="{{url('removesimplesWishList/'.$p->simple_product->id)}}"
                      title="Remove it from your Wishlist" class="removeFrmWish cursor-pointer font-size-25">
                      <i class="fa fa-trash" aria-hidden="true"></i>
                </a>
                 @endif
            ...
            <hr class="orivar{{ $p->simple_product->id }}">
            @endif
            @endforeach
            </ol>
            @else 
...
<script>
     $('.add_in_wish_simple').on('click',function(){
      ...
      $.ajax({
        url : '{{ url("add/simple_pro") }}/' + proid,
        method : 'GET',
        data : {proid : proid},
        success : function(response){
          if(response.status == 'success'){
            session.success(response.msg,'Success');
            ...
            session.error(response.msg,'Failed');
          }
        }
      });
    });
