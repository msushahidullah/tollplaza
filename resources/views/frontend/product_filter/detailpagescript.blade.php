       ...
       if(selling_date <= current_date){
         ...
         if(stock > 0 && stock <= 5)
          ...
          console.log(stock)
              ...
              $('.quantity-container').html('<div><div class="qty-count"><form action="" method="post">{{ csrf_field() }}<div><div class="cart-quantity"><div class="quant-input"></div></div><div class="add-btn"><button type="submit" class="btn btn-primary">{{ __('Add to Cart') }} <i data-feather="briefcase"></i></button></div></div></form></div>');
            ...
            if(guestpricenable == '0' || login_check){
              ...
              $('#cartForm').append('<form action="" method="post">{{ csrf_field() }} <button type="submit" class="btn btn-cart"><i class="fa fa-shopping-cart" aria-hidden="true"></i> {{ __('Add to Cart') }} <span class="sr-only">(current)</span></button></form>');
...
@endphp
        ...
        if(stock > 0 && stock <= 5)
         ...
         $('.quant-input').html('<input type="hidden" name="data_from" value="Checkout" ><input type="number" id="nmovimentos" name="quantity" value="'+data['min_order_qty']+'" min="'+data['min_order_qty']+'" max="'+data['stock']+'" maxorders="'+data['max_order_qty']+'">');

         $('.quant-input').html('');
         $('.quant-input').append('<input type="hidden" name="data_from" value="Checkout" ><input type="hidden" value="'+data['min_order_qty']+'" min="'+data['min_order_qty']+'" max="'+data['stock']+'" maxorders="'+data['max_order_qty']+'" value="1" class="qty-section">');
     @auth
    //  checkwish(varid);
     @endauth
       ...
       success : function(data){
          ...
          if(data == 'InWish'){
           ...
           $('.favorite-button-box').html('');
            ...
            $('.favorite-button-box').append('<a mainid="' + varid + '" data-remove="'+removeWishUrl+'" class="removeFrmWish" data-toggle="tooltip" data-placement="right" title="Remove From Wishlist" href="javascript:"><i data-feather="heart"></i></a>');
          
          }else{
            $('.favorite-button-box').html('');
          
            $('.favorite-button-box').append('<a mainid="' + varid + '" data-add="'+wishurl+'" class="addtowish" data-toggle="tooltip" data-placement="right" title="Add to Wishlist" href="javascript:"><i data-feather="heart"></i></a>');
