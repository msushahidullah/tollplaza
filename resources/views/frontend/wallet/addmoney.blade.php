        ...
        <div class="row">
            <div class="col-lg-12">
                <nav aria-label="breadcrumb" class="breadcrumb-main-block">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ __('Add Money') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
...
<section id="wishlist" class="wishlist-main-block">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="add-money-wallet-block">
                    <h3 class="section-title">Add <i class="{{ $defCurrency->currency_symbol }}"></i> {{ $amount }} via</h3>
                    <hr>
                    <div class="row">
                        @if($configs->paypal_enable == 1)
                            <div class="col-lg-4 mb-3">
                                ...
                                @csrf
                                    ...
                                    <span>{{__('Pay')}} &nbsp;<i class="{{ $defCurrency->currency_symbol }}"></i> {{ $amount }} with</span> 
                        ...
                        <div class="col-lg-4 mb-3">
                            <form action="{{ route('wallet.add.using.instamojo') }}" method="POST">
                                ...
                                <span>{{__('Pay')}} &nbsp;<i class="{{ $defCurrency->currency_symbol }}"></i> {{ $amount }} with <img src="{{ url('images/download.png') }}" alt="instamojo" title="Pay with Instamojo"></span> 
                                </button>
                            </form>
                        </div>
                        @endif
                        ...
                        <div class="col-lg-4 mb-3">
                            <form action="{{ route('wallet.add.using.paytm') }}" method="POST">
                                ...
                                <button type="submit" class="paytm-buy-now-button">
                                    <span>{{ __('Pay') }} &nbsp;<i class="{{ $defCurrency->currency_symbol }}"></i> {{ $amount }} {{ __('with') }} <img src="{{ url('images/paywithpaytm.jpg') }}" alt="paytm" title="Pay with Paytm"></span> 
                                </button>
                            </form>
                        </div>
                        @endif
                        ...
                        @if($configs->razorpay == 1)
                            ...
                            <form id="rpayform" action="{{ route('wallet.add.using.razorpay') }}" method="POST">
                                ...
                                <script src="https://checkout.razorpay.com/v1/checkout.js"
                                        ...
                                        data-buttontext="{{ __('Pay') }} {{ $amount }} {{ __('with') }} Razorpay"
                                        data-name="{{ $title }}"
                        ...
                        @if($configs->stripe_enable == 1)
                            <div class="col-md-12 bg-white">
                                <br>
                                <h4><i class="fa fa-credit-card" aria-hidden="true"></i> {{ __('Pay') }} &nbsp;<i class="{{ $defCurrency->currency_symbol }}"></i> {{ $amount }} (Stripe)</h4>
                                <hr>
                                <div class="row">
                                    <div class="col-md-6">
                                        <form method="POST" action="{{ route('wallet.add.using.stripe') }}" id="credit-card">
                                            ...
                                            <div class="form-group">
                                                <button title="Click to complete your payment !" type="submit" class="btn btn-primary btn-lg btn-block" id="confirm-purchase"> {{ __('ADD') }} <i class="fa {{ $defCurrency->currency_symbol }}"></i>{{ $amount }}</button>
                                            </div>
                                        </form>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="card-wrapper"></div>
                                        <div class="form-container active"></div>
                                    </div>
                                </div>
                            </div>
                        @endif
