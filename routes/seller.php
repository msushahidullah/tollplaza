...
Route::group(['middleware' => ['auth']], function () {
    ...
    Route::prefix('seller')->group(function () {
        ...
        Route::prefix('/gift')->name('seller.gift.')->group(function () {
            // index page 

        Route::get('index','GiftController@index')->name('index');
        // add shipping charge 
        Route::get('create','GiftController@create')->name('create');

        Route::post('store','GiftController@store')->name('store');
        // edit shipping charge
        Route::get('edit/{id}/','GiftController@edit')->name('edit');
        Route::PUT('upadte/{id}',"GiftController@update")->name('update');
        // remove shipping charge 
        Route::delete('/remove/{id}','GiftController@destroy')->name('destroy');
        // ajax request 
        route::any('ajax/request','GiftController@ajaxrequest')->name('ajax.request');

        Route::post('/quickupdate/gift/{id}', 'GiftController@giftUpdate')->name('quick.update');
        });
