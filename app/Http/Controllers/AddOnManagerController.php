...
class AddOnManagerController extends Controller
{
    ...
    public function install(Request $request)
    {
        ...
        if ($zipped) {
            ...
            if ($extract) {
                ...
                return back()->with('added', __(':module Module Installed Successfully', ['module' => $modulename]), __('Installed'));
            }
        }
        $zip->close();
    }
    ...
    public function delete(Request $request)
    {
        ...
        $module = Module::find($request->modulename);
        if (!isset($module)) {
            return back()->with('error', 'Module not found !', '404');
        }
        $module->delete();
        return back()->with('added', 'Module deleted !');
    }
}
