    ...
    public function returnWindow($id)
    {
        ...
        if (Auth::check()) {
            ...
            if (isset($order)) {
                ...
                if ($order->status == 'delivered') {
                    ...
                    if (isset($findvar)) {
                        ...
                        if ($findvar->products->return_avbl == '1' && $findvar->products->return_policy != 0) {
                            ...
                            if ($order->status != 'refunded' || $order->status != 'returned' || $order->status != 'canceled' || $order->status != 'cancel_request') {
                                ...
                                if ($today <= $endOn) {
                                    ...
                                    return redirect()->route('user.view.order', $order->order->order_id)->withErrors(__('Return Policy ended !'))->withInput();
                                }

                            } else {
                              
                                return redirect()->route('user.view.order', $order->order->order_id)->withErrors(__('Product returned already ! Please check once again !'))->withInput();
                            }
                            /*checked*/

                        } else {
                           
                            return redirect()->route('user.view.order', $order->order->order_id)->withErrors(__('Return policy not applicable on this product !'))->withInput();
                        }
                        ...
                        if ($today <= $endOn) {
                            ...
                            session()->flash('error', __('Return Policy ended !'));
                    ...
                    session()->flash('warning', __('Order not delivered yet or already returned !'));
                ...
                session()->flash('error', __('Order not found or already returned !'));
            ...
            session()->flash('error', __('401 | Unauthorized Action !'));
        ...
        if (Auth::check()) {
            ...
            if (Auth::user()->id == $order->order->user_id || Auth::user()->role == 'a') {
                ...
                if (isset($order)) {
                    ...
                    if ($order->status == 'delivered') {
                        ...
                        if ($order->order->payment_method != 'COD') {
                            ...
                            if ($request->source == 'orignal') {
                                ...
                                session()->flash('success', __('Return requested successully ! you will be notifed via email once we get the product and refund will proceed at same day !'));
                        ...
                        session()->flash('warning', __('Product is not delivered yet !'));
                    ...
                    session()->flash('error', __('404 | Order Not found !'));
                ...
                session()->flash('error', __('404 | Unauthorized action !'));
