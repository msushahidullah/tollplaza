...
class CartController extends Controller
{
    ...
    public function index($id)
    {
        ...
        if (!empty($user)) {
            ...
            if (empty($cart_table)) {
                ...
                return back()->with('error',__('Failure'), __('This Product is Already Available'));
            }
            ...
            if ($cart_table) {
                ...
                if ($cart_table->variant_id == $variantid) {
                    ...
                    if ($tqty <= $limit) {
                        ...
                        session()->flash('warning', 'Product already in cart with max quantity limit!');
                        return redirect('cart');
    ...
    public function remove_cart($id)
    {
        ...
        if (!Session::has('coupanapplied')) {
            ...
            if (count(Session::get('cart')) > 0) {
                ...
                session()->flash('success', 'Item is removed from your cart !');
    ...
    function empty() {
        ...
        return redirect('/')->with('added', 'Your cart is now empty');
    ...
    public function simpeproductincart(Request $request, $pro_id, $price, $offerprice)
    {
        ...
        if (!$cart) {
            ...
            if ($qty > $product->max_order_qty) {
                ...
                session()->flash('error', 'Product max qty limit reached !');
                return back();
        ...
        session()->flash('success', 'added in cart', $product->product_name);
