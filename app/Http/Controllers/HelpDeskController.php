...
{
    $request->validate([
        'issue_title' => 'required|min:6',
        'issue' => 'required|max:5000',
        'image' => 'mimes:png,jpeg,jpg,bmp,gif'
    ]);

    $authUser = Auth::user();
    if (!$authUser) {
        return back()->withErrors(__('User not authenticated.'));
    }
    ...
    session()->flash('success', __('Ticket has been created! You can view the status of your ticket under My Account.'));
    ...
    public function replay(Request $request, $id)
    {
        ...
        try {
            Mail::to($get_user_email)->send(new SendTicketReplay($hd, $newmsg));
               ...
               session()->flash('success', __('Reply sent !'));
