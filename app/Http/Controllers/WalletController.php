...
class WalletController extends Controller
{
    ...
    public function showWallet(Request $request)
    {
        ...
        if (isset(Auth::user()->wallet)) {
            ...
            if (Auth::user()->wallet->status == 1) {
                ...
                session()->flash('error', __('Sorry your wallet is not active !'));
                return back();
            }
        ...
        if ($response->getState() == 'approved') {
            ...
            if (isset($wallet)) {
                ...
                if ($wallet->status == 1) {
                 ...
                 session()->flash('error', __('Your wallet is not active yet ! contact support system !'));
                    return back();
                }
    ...
    public function addMoneyViaInstamojo(Request $request)
    {
        ...
        try
        {
            ...
            $failedTranscations->user_id = Auth::user()->id;
            $failedTranscations->save();
             session()->flash('error', __('Payment Failed ! Try Again'));
            return redirect()->route('user.wallet.show');
        }
    ...
    public function addMoneyViaStripe(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            if ($stripe == '' || $stripe == null) {
                session()->flash('error', __("Stripe Key Not Found Please Contact your Site Admin !"));
                return redirect()->route('user.wallet.show');
            }
            ...
            try {
                ...
                if (!isset($token['id'])) {
                   session()->flash('error', __('The Stripe Token was not generated correctly !'));
                    return redirect()->route('user.wallet.show');
                }
            ...
            session()->flash('error', __("All fields are required !"));
            return redirect()->route('user.wallet.show');
        }
    }
    ...
    public function brainConfig()
    {
        ...
        return new Braintree\Gateway([
            'environment' => env('BRAINTREE_ENV') != '' ? env('BRAINTREE_ENV') : 'sandbox',
    ...
    public function checkout(Request $request)
    {
        ...
        if (round($request->actualtotal, 2) != round($total, 2)) {
            session()->flash('error', __('Payment has been modifed !', 'Please try again !'));
        ...
        // try {
            ...
            if($neworder->discount != 0 ){
                $gtotal = $neworder->order_total-$neworder->discount+$neworder->handlingcharge;
            }else{
               $gtotal = $neworder->order_total+$neworder->handlingcharge; 
            }
            ...
            try {
                ...
                if (isset($e) && $e->email) {
                    Mail::to($e->email)->send(new OrderMail($neworder, $inv_cus, $paidcurrency, $gtotal));
                }
