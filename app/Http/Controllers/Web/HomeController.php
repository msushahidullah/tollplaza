...
class HomeController extends Controller
{
    ...
    {
        require base_path() . '/app/Http/Controllers/price.php';  // Load price-related data
    
        $offersettings = OfferPopup::first();  // Get offer popup settings
        $bannersetting = BannerSetting::first();  // Get banner settings
     $newproduct = $this->getTabbedProducts();  // Get tabbed products
        $featuredproducts = $this->featuredProducts();  // Get featured products
    
        // Get language settings from session
        $lang = session()->get('changed_language');
        $fallback_local = config('translatable.fallback_locale');  // Fallback language
        ...
        // Get top category products and blogs
        $topcatgoryproducts = $this->topProducts();
        $blogs = $this->blog();
      $sliders = $this->slider();
      $sidebarcategories  = $this->sidebarcategories();
      $cats = Category::where('status', '1')->orderBy('position')->get();
        // Return view with all necessary data
        return view('front.homepage', compact('conversion_rate',
            'offersettings', 'bannersetting', 'newproduct', 'featuredproducts', 
            'data', 'topcatgoryproducts', 'blogs','sliders','sidebarcategories','cats'
        ));
    }
    ...
    public function simple_products(){
        ...
        return $simple_products->map(function($content) use($conversion_rate) {
            ...
            $content['cartURL']  = route('add.cart.simple',['pro_id' => $content->id, 'price' => $content->price, 'offerprice' => $content->offer_price ?? 0, 'qty' => $content->min_order_qty]);
            $content['pre_order']           = $content->pre_order;
            $content['preorder_type']       = $content->preorder_type;
            $content['partial_payment_per'] = $content->partial_payment_per;
            $content['product_avbl_date']   = $content->product_avbl_date;
    ...
    public function slider()
    {
        ...
        return ['enable' => $enable ,'sliders' => $sliders, 'lang' => $lang, 'fallbacklang' => $fallback_local];
    }

    public function slider2()
    {

        $sliders = Slider2::where('status', '=', '1')->get();
        ...
        $sliders = $sliders->map(function ($query) {
            ...
            $content['image'] = url('images/slider2/' . $query->image);
            $content['topheading'] = $query->getTranslations('topheading');
            $content['headingtextcolor'] = $query->headingtextcolor;
            $content['heading'] = $query->getTranslations('heading');
            $content['subheadingcolor'] = $query->subheadingcolor;
    ...
    public function slider3()
    {
        ...
        $sliders = Slider3::where('status', '=', '1')->get();
        ...
        $sliders = $sliders->map(function ($query) {
            ...
            $content['image'] = url('images/slider3/' . $query->image);
            $content['topheading'] = $query->getTranslations('topheading');
            $content['headingtextcolor'] = $query->headingtextcolor;
            $content['heading'] = $query->getTranslations('heading');
            $content['subheadingcolor'] = $query->subheadingcolor;
    ...
    public function blog()
    {
        ...
        try {
            return $blogs->map(function ($q) {
                $q->heading = [
                    'en' => $q->heading, // Assuming the `heading` field contains the default language value
                ];
  ...
  public function topProducts()
{
    ...
    if (isset($top_categories) && !empty($top_categories->category_ids)) {
        $categoryIds = is_string($top_categories->category_ids)
            ? json_decode($top_categories->category_ids, true) // Decode if it's a string
            : $top_categories->category_ids; // Use as is if it's already an array

        foreach ($categoryIds as $categoryId) {
            $category = Category::where('status', '1')->where('id', $categoryId)->first();
    ...
    public function categoryproducts($top_categories, $category)
    {
        ...
        $topcatproducts = $topcatproducts->map(function ($q) use ($get_product_data, $content, $conversion_rate) {
            ...
            if($orivar->def == 0){
                $orivar = AddSubVariant::where('def', 1)->where('pro_id', $q->id)->first();
            }
    ...
    public function getTabbedProducts()
{
    ...
    // Simplify the check
    $othercats = $newproductcat ? explode(",", $newproductcat->name) : [];

    $cats = [];
    $all = [
        'products' => $this->getProducts('all') // Ensure this method exists and works
    ];

    // If there are other categories, fetch them
    if ($othercats) {
    ...
    // If you're rendering a view, pass the data like this:
    return ['all' => $all, 'cats' => $cats];
}
    ...
    public function sidebarcategories()
    {
        ...
        ->where('categories.status', '=', '1')->with(['subcategory' => function ($query) {
            ...
            return $query->select('parent_cat', 'id', 'title', 'icon','status');

        }, 'subcategory.childcategory' => function ($q) {
            $q->where("status","=","1");
            return $q->select('subcat_id', 'id', 'title','status');
        }])->get();
        ...
        if($sp_specialoffers){
            $sp_specialoffers = $sp_specialoffers->map(function($sp) use($conversion_rate,$content) {
                ...
                $content['symbol'] = session()->get('currency')['value'];
                $content['position'] = session()->get('currency')['position'];
                $content['cartURL'] = route('add.cart.simple',['pro_id' => $sp->id, 'price' => $sp->price, 'offerprice' => $sp->offer_price, 'qty' => $sp->min_order_qty]);
    ...
    public function topmenus()
    {
        ...
        return ['menus' => $menus, 'lang' => $lang, 'fallback_local' => $fallback_local];
    }
    ...
    public function totalCart()
    {
        try {
            ...
            if(count($handling_charge) ==$count_handling_charge){
                if(isset($handling_charge[0])){
                    $handling_amount = $handling_charge[0]->global_price;
                }
                else{
                    $handling_amount = 0;
                }
            ...
            return [
                ...
                "payment_handling_charge"=>$handling_charge
            ];
    ...
    public function add_or_removewishlist(Request $request)
    {
        ...
        public function wishlistcount($asJson = true)
{
    try {
        if (Auth::check()) {
            $data = Wishlist::with([
                ...
                ->where(function ($query) {
                    $query->whereHas('variant')
                          ->whereHas('variant.products', function ($subQuery) {
        ...
        // If AJAX → return JSON, else → return integer
        return $asJson 
            ? response()->json(['wishlist_count' => $data]) 
            : $data;
    ...
    public function comparecount()
    {
        ...
        return Session::get('comparison') != null ? count(Session::get('comparison')) : 0;
    }
