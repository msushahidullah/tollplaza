...
class BackupController extends Controller
{
    ...
    public function updatedumpPath(Request $request){
        ...
        session()->flash('success', 'SQL dump path updated !');
    ...
    public function process(Request $request){
        ...
        if(env('DEMO_LOCK') == 1){
            ...
            return back()->with('error','This action is disabled in demo !');
        }
        ...
        session()->flash('success', __('Backup completed !'),__('Done !'));
    ...
    public function download(Request $request, $filename){
        ...
        if (! $request->hasValidSignature()) {
             return redirect(route('admin.backup.settings'))->with('error','Download Link is invalid or expired !');
        }
