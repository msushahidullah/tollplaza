...
class SellerCancelOrderController extends Controller
{
   ...
   public function __construct() {
    ...
    $secret   = $paypal_conf['secret'] ?? null;
    ...
    if ($clientId && $secret) {
        ...
        // optional: log or throw error for missing PayPal config
        Log::warning("PayPal configuration missing. Please set client_id and secret in config/paypal.php");
    }
}
                    ...
                    elseif ($order->payment_method == 'Paytm') {
                        ...
                        else if ($refund->isFailed())
                        {
                            ...
                            if ($response['STATUS'] == 'TXN_FAILURE')
                            {
                                ...
                                return back()->withErrors($response['RESPMSG']);
