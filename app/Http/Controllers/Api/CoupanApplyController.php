...
class CoupanApplyController extends Controller
{
    ...
    public function validCouponForCart($currency,$cpn)
    {
        ...
        $paymentdetail = new CartController;
            ...
            if (isset($cart))
            {
                ...
                if ($cpn->minamount != 0)
                {
                    ...
                    if ($total*$conversion_rate >= $cpn->minamount*$conversion_rate)
                    {
                        ...
                        $pd = $paymentdetail->cartTotal()->getData();

                        return response()->json([
                            'msg'               => "$cpn->code Applied Successfully !" ,
                            'status'            => 'success',
                            'subtotal'          => $pd->subtotal * $conversion_rate,
                            'shipping'          => $pd->shipping * $conversion_rate,
                            'grand_total'       => $pd->grandTotal,
                            'coupan_discount'   => $per * $conversion_rate,
                        ]);
                    }
                    else
                    {
        ...
        if (Auth::check())
        {
            ...
            if (isset($cart))
            {
                ...
                if ($cart->simple_pro_id == $cpn->simple_pro_id)
                {
                    ...
                    Cart::where('simple_pro_id', '=', $cpn['simple_pro_id'])
    ...
    public function removeCoupan(Request $request){
        ...
        if(Auth::check()){
            ...
            return response()->json([
                'msg'               => "$cpn->code removed Successfully !" ,
                'status'            => 'success',
                'subtotal'          => $pd->subtotal    * $conversion_rate,
                'shipping'          => $pd->shipping    * $conversion_rate,
                'grand_total'       => $pd->grandTotal,
