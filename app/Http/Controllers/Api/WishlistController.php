...
class WishlistController extends Controller
{
    ...
    public function deleteCollection(Request $request, $id)
    {
        ...
        if (WishlistCollection::where('id', $id)->exists()) {
            $data = WishlistCollection::find($id);
            $data->delete();
    ...
    public function topCollectionItemImages($coll)
    {
        ...
        if ($coll != '0') {
            ...
            auth()->user()->wishlist()->take(6)->each(function ($item) use (&$images) {
    ...
    public function collectionItem($collection, $currency)
    {
        ...
        foreach ($collection->items as $item) {
            ...
            $wishlistItem[] = array(
                ...
                'price' => (float) sprintf('%.2f', $price->mainprice * $rate->exchange_rate),
                'offerprice' => (float) sprintf("%.2f", $price->offerprice * $rate->exchange_rate),
