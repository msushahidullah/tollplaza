...
public function showCompletePayout($id, PayPalClient $paypal)
{
    ...
    if ($payout->paidvia == 'Bank' || $payout->txn_type == 'manual') {
        ...
        // Correct method name for v3.x
        $response = $paypal->showBatchPayoutDetails($payout->txn_id);

        if (!empty($response) && isset($response['batch_header'])) {
...
public function printSlip($id, PayPalClient $paypal)
{
    $payout = SellerPayout::findOrFail($id);
    $inv_cus = Invoice::first();
