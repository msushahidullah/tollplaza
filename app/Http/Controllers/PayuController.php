...
class PayuController extends Controller
{
    ...
    public function payment($order_id,$amount,$name,$email,$phone,$purpose,$error)
    {
        ...
        if (Session::get('currency')['id'] != 'INR') {
            ...
            session()->flash('error', __('Currency is in ' . strtoupper(Session::get('currency')['id']) . ' and payumoney only support INR currency.'));
    ...
    public function status()
    {
        ...
        if ($payment->successful()) {
            ...
            session()->flash('error', __('Payment not done due to some payumoney server issue !'));
