        ...
        if ($pincodesystem == 1) {
            ...
            if (strlen($getpincode) > 5) {
                ...
                if (empty($avbl_pincode->pincode)) {
                    ...
                    return redirect('/checkout')->with('error','Delivery not available on selected address pincode !');
                }
            ...
            foreach ($cart_table as $carts) {
                ...
                if (isset($pros)) {
                    ...
                    if ($max >= $min) {
                        ...
                        session()->flash('error', __('Sorry the product is out of stock !'));
                        return back();
                ...
                if (isset($cart->simple_product)) {
                    ...
                    session()->flash('error', __(':product is out of stock now!', ['product' => $cart->simple_product->product_name]));
                        return back();
    ...
    public function add(Request $request)
    {
        ...
        if (auth()->user()->billingAddress()->count()) {
            ...
            if ($request->sameship == 1) {
                ...
                if ($request->billing_name != '' && $request->billing_address != '' && $request->billing_mobile != '' && $request->billing_state != "" && $request->billing_city != '' && $request->billing_country != '' && $request->billing_email != '') {
                    $newbilling = new BillingAddress();
                    ...
                    session()->flash('error', __('Please fill all fields to continue !'));
                    return back();
                }
        ...
        return redirect(route('order.review'))->with('added','Billing address updated successfully !');
...
//         $cart_table = $cart_table = Cart::where('user_id',auth()->id())->where('active_cart',1)->with(['simple_product','product','product.reviews','variant','product.shippingmethod'])
//         ->orWhereHas('simple_product')
//         ->whereHas('product',function($query){
//             return $query->where('status','1');
//         })->whereHas('variant')->get();
...
//       */
//      $shippingChage = null;
//      $shippingAddress = ShippingCharge::where("city_id",$selectedaddress->getcity->name)->first();
//      if($shippingAddress){
...
//         return view('front.checkout', compact('shippingChage','selectedaddress', 'selectedstates', 'selectedcities', 'shippingcharge', 'cart_table', 'addresses', 'all_country', 'conversion_rate', 'sentfromlastpage','handling_charge','handing_charge_array','shipping_coupan', 'shipping_coupan_type', 'shipping_coupan_per_price', 'shipping_coupan_status', 'ctotal'));
//     }

public function orderReview()
{
    $handing_charge_array = [];
    $genrals_settings = Genral::first();

    $shippingCoupan = ShippingCoupan::first();
    $shipping_coupan = $shippingCoupan->name ?? null;
    $shipping_coupan_type = $shippingCoupan->coupan_type ?? null;
    $shipping_coupan_per_price = $shippingCoupan->number_of_price ?? null;
    $shipping_coupan_status = $shippingCoupan->status ?? null;
    ...
    if (!empty($handling_charge[0])) {
        if (count($handling_charge) == $count_handling_charge) {
            $genrals_settings?->update(['handlingcharge' => $handling_charge[0]->global_price ?? 0]);
        } else {
            foreach ($handling_charge as $handlingcharge) {
                $handing_charge_array[strtolower($handlingcharge->payment_getway_name ?? '')] = $handlingcharge->price ?? 0;
            }

            $paypalCharge = HandlingCharge::where('payment_getway_name', 'paypal')->first();
            $genrals_settings?->update(['handlingcharge' => $paypalCharge->price ?? 0]);
        }
    }
    ...
    $cart_table = Cart::where('user_id', auth()->id())
        ->where('active_cart', 1)
        ->with(['simple_product','product','product.reviews','variant','product.shippingmethod'])
        ->orWhereHas('simple_product')
        ->whereHas('product', function ($query) {
            return $query->where('status', '1');
        })
    ...
    $shippingcharge = session()->get('shippingcharge') ?? 0;

    $all_country = Allcountry::join('countries', 'countries.country', '=', 'allcountry.iso3')
        ->select('allcountry.*')
        ->get() ?? collect();

    $selectedstates = DB::table('allstates')
        ->where('country_id', Session::get('billing')['country_id'] ?? 0)
        ->get() ?? collect();

    $selectedcities = DB::table('allcities')
        ->where('state_id', Session::get('billing')['state'] ?? 0)
        ->get() ?? collect();

    $selectedaddress = Address::with(['getCountry', 'getstate', 'getcity'])
        ->whereHas('getCountry')
        ->whereHas('getstate')
        ->find(Session::get('address')) ?? null;

    /** Shipping charge feature */
    $shippingChage = 0;
    if ($selectedaddress?->getcity?->id) {
        $shippingAddress = ShippingCharge::where("city_id", $selectedaddress->getcity->id)->first();
        if ($shippingAddress) {
            $shippingChage = $shippingAddress->Type_of_charge === "global"
                ? ($shippingAddress->global_price ?? 0)
                : ($shippingAddress->custom_price ?? 0);
        }
    }
    ...
    $cart_table->each(function ($cart) use ($count) {
        if ($cart->active_cart == 1) {
            ...
            if (get_default_shipping()?->whole_order == 1) {
                if ($cart->ship_type != 'localpickup' && $cart->product?->free_shipping == 0) {
                    $count->push(1);
                }
                if ($cart->simple_product && $cart->ship_type != 'localpickup' && $cart->simple_product->free_shipping == 0) {
                    $count->push(1);
    ...
    if (count($count)) {
        $cart_table->each(function ($cart) use ($count) {
            if ($cart->active_cart == 1 && get_default_shipping()?->whole_order == 1) {
                if ($cart->ship_type != 'localpickup') {
                    if ($cart->variant && $cart->product) {
                        $cart->shipping = (float) ShippingPrice::calculateShipping($cart) / $count->count();
                        $cart->save();
    ...
    $ctotal = 0;
    foreach ($cart_table as $crt) {
        if ($crt->active_cart == 1) {
            $ctotal += ($crt->semi_total ?? $crt->price_total ?? 0) + ($crt->shipping ?? 0);
        }
    }

    if ($ctotal && $genrals_settings?->cart_amount) {
        if ($ctotal * ($conversion_rate ?? 1) >= $genrals_settings->cart_amount * ($conversion_rate ?? 1)) {
            DB::table('carts')->where('user_id', auth()->id())->update(['shipping' => 0]);
        }
    }
    ...
    return view('front.checkout', compact(
        'shippingChage','selectedaddress','selectedstates','selectedcities',
        'shippingcharge','cart_table','addresses','all_country','conversion_rate',
        'sentfromlastpage','handling_charge','handing_charge_array','shipping_coupan',
        'shipping_coupan_type','shipping_coupan_per_price','shipping_coupan_status','ctotal'
    ));
}
