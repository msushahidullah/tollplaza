...
class CashfreeController extends Controller
{
    ...
    public function pay($order_id,$amount,$name,$email,$phone,$purpose,$error)
    {
        ...
        if (session()->get('currency')['id'] != 'INR') {
            ...
            session()->flash('error', __('Cashfree Only Support INR Payment !'));
        ...
        if ($c < 10) {
            ...
            session()->flash('error', __('Invalid Phone no.'));
        ...
        if ($jsonResponse->{'status'} == "OK") {
            ...
            session()->flash('error', $jsonResponse->{'reason'});
    ...
    public function success(Request $request)
    {
        ...
        if ($request->txStatus == 'CANCELLED') {
            ...
            session()->flash('warning', $request->txMsg);
