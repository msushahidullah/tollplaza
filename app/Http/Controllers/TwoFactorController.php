...
class TwoFactorController extends Controller
{
    ...
    public function generate2faSecret(Request $request){
     ...
     session()->flash('success', __('Secret key is generated, Please verify code to enable 2FA'));
    ...
    public function valid2FA(Request $request){
        ...
        if($google2fa->verifyKey($user->google2fa_secret, $request->one_time_password)){
           ...
           session()->flash('success', __('2FA is enabled on your account !'));

            return back();

        }else{
           session()->flash('error', __('Secret Key is invalid, Please verify Code again to Enable 2FA'));
               return back();
        }
    ...
    public function disable2FA(Request $request){
        ...
        if(Hash::check($request->password,auth()->user()->password)){
            ...
            session()->flash('success', __('2FA is disabled in your account !'));
