...
class InstallerController extends Controller
{
    ...
    public function storeeula(Request $request)
    {
        if (isset($request->eula)) {
            ...
            return back()->withErrors('Please Accept Terms and conditions first !');
        }
    }
   ...
   public function getstep3()
{
    try {
        ...
        if (env('IS_INSTALLED') == 0) {
            ...
            $getstatus = Crypt::decrypt($getstatus);
            Log::info('Step3: Status file decrypted', ['status' => $getstatus]);

            if ($getstatus == 'complete') {
                Log::info('Step3: Installation complete, showing step3 view.');
                return view('install.step3');
            }
        } else {
            Log::info('Step3: Already installed, redirecting to home.');
            return redirect('/');
        }
    } catch (\Exception $e) {
        Log::error('Step3: Error occurred', [
        ...
        return redirect()->route('get.step2')->withErrors($e->getMessage(), $e->getCode());
    }
}
    ...
    public function storeStep5(Request $request)
    {
        ...
        if ($apistatus != 1) {
            ...
            Artisan::call('cache:clear');
            Artisan::call('view:clear');
            return redirect()->route('get.step5')->withErrors(__('Oops Please try again !'));
        }
        Session::flush();
    ...
    {
        // Load stored  license
        $token = file_exists(storage_path('app/keys/license.json')) && file_get_contents(storage_path('app/keys/license.json')) != null
            ? file_get_contents(storage_path('app/keys/license.json'))
            : '';
        ...
        if (!empty($token) && isset($token->token)) {
            ...
            return 2; // No license found
        }
        // Get current domain
        $domain = str_replace("www.", "", \Illuminate\Support\Facades\Request::getHost());
        // Verify license with  API
        $ch = curl_init();
        $options = [
            CURLOPT_URL => "https://api..com/v3/market/author/sale?code={$purchaseCode}",
        ...
        if ($responseCode == 200) {
            ...
            if (!empty($body['item'])) {
                // License is valid → update stored file with new status
                $lic_json = [
                ...
                Storage::disk('local')->put('/keys/license.json', json_encode($lic_json, JSON_PRETTY_PRINT));
                return 1; // success
            }
        }
