...
class SellerPaymenyController extends Controller
{
        public function manualPayout(Request $request, $venderid, $orderid)
    {
        ...
        if (isset($check)) {
            session()->flash("warning",__("For this order payout is already done to the seller"));
            ...
            session()->flash("success",__("Payout record for that order stored !"));
    ...
    public function track(Request $request, $batchid)
    {
        if ($request->ajax()) {
            ...
            $response = $paypal->showBatchPayoutDetails($batchid); // Correct method for batch payout
    ...
    public function payoutviabank(Request $request, $venderid, $orderid)
    {
        ...
        if (isset($check)) {
            ...
            $order->delete();
            session()->flash("success",__("Payout for that order completed successfully !"));
    ...
    public function showCompletePayout($id)
{
    ...
    if ($payout->paidvia == 'Bank' || $payout->txn_type == 'manual') {
        ...
        try {
            $response = $paypal->showBatchPayoutDetails($payout->txn_id);
            $status = $response['batch_header']['batch_status'] ?? 'Unknown';
            $txnid = $response['items'][0]['transaction_id'] ?? null;
        } catch (\Exception $e) {
            $status = 'Error';
            $txnid = null;
            \Log::error("PayPal Payout Fetch Error: " . $e->getMessage());
        }
    }
...
public function printSlip($id)
{
     ...
     */
     ...
     public function payoutviaPaypal($findstore, $amount, $order, $defCurrency)
     {
         ...
         $payoutData = [
             ...
             'items' => [
                 [
                     ...
                     'receiver'       => $sendemail,
                     'note'           => __('Thanks for using our portal for selling your product!'),
         ...
         try {
             // Create Payout
             $response = $paypal->createPayout($payoutData);
     
             // Fetch Payout Details
             $batchId = $response['batch_header']['payout_batch_id'];
             $payoutDetails = $paypal->showBatchPayoutDetails($batchId);
     
             DB::beginTransaction();
     
             // Store Payout Data in Database
             $newpayout = new SellerPayout();
             $newpayout->payoutid = str()->random(6);
             $newpayout->orderid = $order->singleorder->id;
             $newpayout->sellerid = $order->sellerid;
             $newpayout->paidby = auth()->id();
             $newpayout->paid_in = $payoutDetails['batch_header']['amount']['currency'];
             $newpayout->orderamount = $payoutDetails['batch_header']['amount']['value'];
             $newpayout->txn_fee = $payoutDetails['batch_header']['fees']['value'];
             $newpayout->txn_id = $payoutDetails['batch_header']['payout_batch_id'];
             ...
             $item = $inv_cus->prefix . $order->singleorder->inv_no . $inv_cus->postfix;
             $msg = "You Received a new Payout for Order Item #$item";
             $currency = $payoutDetails['batch_header']['amount']['currency'];
             $amount = $payoutDetails['batch_header']['amount']['value'];
             $url = route('vender.payout.show.complete', $newpayout->id);
             $type = 'paypal';
             ...
             if ($order->singleorder->order->payment_method == 'Wallet') {
                 ...
                 $adminwalletlog = new OrderWalletLogs();
                 $adminwalletlog->wallet_txn_id = $payoutDetails['batch_header']['payout_batch_id'];
                 $adminwalletlog->note = "Payout to seller for invoice no. #$item";
                 $adminwalletlog->type = 'Debit';
                 $adminwalletlog->amount = $payoutDetails['batch_header']['amount']['value'];
                 $adminwalletlog->balance = isset($lastbalance) ? $lastbalance->balance - $payoutDetails['batch_header']['amount']['value'] : $payoutDetails['batch_header']['amount']['value'];
                 $adminwalletlog->save();
             }
     
             InvoiceDownload::where('id', $order->singleorder->id)->update(['paid_to_seller' => 'YES']);
     
             // Send Notification
             $seller = User::find($order->sellerid);
             if ($seller) {
                 $seller->notify(new SellerNotification($url, $msg));
     
                 // Send Email
                 try {
                     $emailTo = $sendemail ?: $seller->email;
                     Mail::to($emailTo)->send(new PayoutNotify($msg, $currency, $amount, $type));
                 } catch (\Exception $e) {
                     \Log::error("Mail Error: " . $e->getMessage());
                 }
             }
     
             // Remove pending payout entry
             $order->delete();
             ...
             Session::flash('error', __("PayPal payout failed!"));
             return redirect()->back();
         }
        }
    public function payoutviaStripe()
    {
    Session::flash('info', __('Coming soon !'));        
        return back();
    }
